! Vectorization-fix variants of set_residual_damp. The parent's scaling
! loop only reaches PARTIAL LOOP WAS VECTORIZED under ifort, because the
! main sweep also writes dU and ifort cannot disprove that the two write
! regions overlap ("assumed OUTPUT dependence between J and J"), so it
! distributes the nest. Production's standalone damp_residual has an
! identical loop and vectorizes cleanly -- so the problem is scope, not the
! loop. Two candidate fixes, both arithmetically identical to the parent.
subroutine set_residual_damp_ivdep( &
    cons, P, P_offset, &
    r, Omega, dAi, dAj, dAk, &
    f_body, &
    dU, &
...
end subroutine set_residual_damp_ivdep


subroutine set_residual_damp_split( &
    cons, P, P_offset, &
    r, Omega, dAi, dAj, dAk, &
    f_body, &
    dU, &
...
end subroutine set_residual_damp_split
