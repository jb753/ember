! EXPERIMENTAL, bench only. Half of viscous_tauq_packed: the TRANSPORT
! properties are packed component-first, the velocities are left alone.
!
! Packing everything came out 8.8% SLOWER than the parent, which the layout
! explains: i is the vectorised axis, so Vx(i,j,k) loads at unit stride and
! vel(1,i,j,k) at stride 3, and the velocity components are read in three
! separate blocks with nothing to recover the packing from.
!
! mu, cp and kappa are different: stage 1 corner-averages all three over the
! SAME eight nodes on consecutive lines, so a packed node's three components
! share a cache line. If the rule is "pack what is read together", this half
! should win where the other half lost.
!
...
end module viscous_ptr_helpers


subroutine set_visc_force_tqf_ptrans( &
...
end subroutine set_visc_force_tqf_ptrans
