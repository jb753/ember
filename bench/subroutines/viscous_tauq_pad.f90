! TIMING CONTROL ONLY. Correct, but it exists to answer one question.
!
! viscous_tauq_ctl beats viscous_tauq_fused by 13.8% while doing strictly MORE
! work -- an extra plane of producer in its pre-pass -- with DOUBLE the rolling
! buffer. Nobody predicted that and nothing explains it. But ctl differs from
! its parent in two ways at once, the pre-pass and the four-slot tq, so the
! measurement cannot say which.
!
! This arm is set_visc_force_tqf with ONE character changed: its tq dummy is
! declared with four slots instead of two. It still uses only slots 1 and 2,
! still has no pre-pass, and computes exactly what its parent computes. The
! only thing that moves is how much buffer the caller hands it and therefore
! where that buffer sits.
!
!   lands near tqf   (48.5)  ->  the pre-pass is doing it, and it is codegen
!   lands near ctl   (41.8)  ->  the buffer is doing it, and it is data layout
!
...
end module viscous_pad_helpers


subroutine set_visc_force_tqf_pad( &
...
end subroutine set_visc_force_tqf_pad
