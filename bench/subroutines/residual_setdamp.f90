...
! on speed.
!
! The scaling pass lives in a contained subroutine rather than inline. That
! is not cosmetic: inline, ifort sees the main sweep and the scaling loop
! both writing dU, cannot disprove the write regions overlap ("assumed
! OUTPUT dependence"), and distributes the nest into a PARTIAL
! vectorization. Contained, dU is the only array in scope and the loop
! vectorizes cleanly. Measured worth: nothing (+0.20% median, 43/100 ranks
! -- a tie), so this is codegen hygiene, not a speedup. See section 24;
! !DIR$ IVDEP on the inner loop does NOT fix it, because the dependence is
! between two loop nests, not carried within one.
!
