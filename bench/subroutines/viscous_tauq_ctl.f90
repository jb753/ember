! TIMING CONTROL ONLY -- NOT CORRECT, NEVER GATE IT.
!
! set_visc_force_tqf_selfk with the seam select put back to the parent
! arm's two load_halo_kplane calls, but keeping the pre-pass and the
! 4-slot tq. It exists to answer one question: the seam-free arm beats
! its parent by ~5.8 ns/cell, flat across a 6.7x size range, while the
! exchange it removes is worth only 0.63 ns/cell. This arm splits that
! gap between the seam select and the pre-pass/buffer change.
!
! It needs the exchange to be correct (it reads the halo again) and it
! also pays the pre-pass, so it is nobody's candidate implementation.
!
...
module viscous_ctl_helpers
    implicit none
    private
    public :: load_halo_ijedge, load_halo_kplane
...
end module viscous_ctl_helpers


subroutine set_visc_force_tqf_ctl( &
    ...
    if (k <= nk-1) then
        ...
        if (k == 1) then
            call load_halo_kplane(tau_cell, q_cell, tq(1,1,1,ta), &
                wallk1, 1, ni, nj, nk)
        end if
    else
        call load_halo_kplane(tau_cell, q_cell, tq(1,1,1,tb), &
            wallnk, nk+1, ni, nj, nk)
    end if
...
end subroutine set_visc_force_tqf_ctl
