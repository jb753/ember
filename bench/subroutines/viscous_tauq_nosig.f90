...
!
! set_visc_force_tqf_noij with tau_cell/q_cell removed from the
! SIGNATURE, not merely left unread. The full-volume tau/q buffer is
! not passed, not declared and not mapped into the subroutine at all.
!
! Pairing it with _noij separates two things that were conflated: the
! cost of READING the 37.8 MB buffer inside the k loop (_selfk vs
! _noij) from the cost of it merely being an argument (_noij vs this).
! Note what neither measures: the block still ALLOCATES the buffer and
! seed_tau_q still fills it, so the case for a compact surface buffer
! rests on allocation and on phase 1's writes, which no arm here models.
!
! TIMING CONTROL ONLY -- WRONG ANSWER BY CONSTRUCTION, NEVER GATE IT.
!
...
end module viscous_nosig_helpers


subroutine set_visc_force_tqf_nosig( &
...
end subroutine set_visc_force_tqf_nosig
