...
#SBATCH -o irsbj_bench-%j.out
#SBATCH -e irsbj_bench-%j.err
...
RESULTS="${SUBMIT_DIR}/tools/bench_irs_bj.jsonl"
OPT="${SUBMIT_DIR}/tools/opt_report_irs_bj.txt"
...
srun --qos=intr -n1 -c1 --cpu-bind=cores \
    python3 tools/bench_irs_bj.py --mode serial --reps 40 --json "$RESULTS"
...
srun --qos=intr -n100 -c1 --cpu-bind=cores \
    python3 tools/bench_irs_bj.py --mode saturated --reps 40 --json "$RESULTS"
...
for mode in ("serial","saturated"):
    ...
    if not sel: continue
    names=list(sel[0]["results"]); base=names[0]
    print(f"\n--- {mode} ({len(sel)} rank(s)) ---")
