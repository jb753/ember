...
def test_set_P_accepts_a_scalar_and_a_spanwise_profile():
    """The two shapes that mean something: uniform, and one value per span station.

    What is stored either way is one value per span station, since the
    ...
    np.testing.assert_allclose(patch.P_nd.ravel(), profile / FLUID.P_ref, rtol=1e-6)
...
def test_set_P_accepts_a_bare_1d_span_profile():
    """A plain length-nspan array is a spanwise profile, laid on the span axis.

    Unambiguous because pitchwise variation is refused outright, so a
    one-dimensional prescription has nothing else it could mean. It must give
    the same target as the same profile written with the patch's own axes --
    in particular it must land along the span, not wherever numpy's
    trailing-axis alignment would have put it.
    """
    block = make_block()
    patch = OutletPatch(i=-1)
    block.patches.append(patch)
    ...
    patch.set_P(profile)
    flat = np.copy(patch.P_nd)

    shape = [1, 1, 1]
    shape[patch.span_dim] = nspan
    patch.set_P(profile.reshape(shape))

    np.testing.assert_allclose(flat, patch.P_nd, rtol=1e-6)
    np.testing.assert_allclose(flat.ravel(), profile / FLUID.P_ref, rtol=1e-6)


@pytest.mark.parametrize(
    "kind", ["pitchwise", "full_face", "wrong_length", "wrong_rank"]
)
def test_set_P_rejects_shapes_that_are_not_a_span_profile(kind):
    """Anything that is not a scalar or a spanwise profile is refused.

    A pitchwise-varying value in particular is rejected rather than averaged:
    accepting it would impose its mean while reading as a node-by-node
    prescription.
    """
    block = make_block(nspan=7, npitch=17)
    patch = OutletPatch(i=-1)
    block.patches.append(patch)
    nspan = patch.shape[patch.span_dim]
    npitch = patch.shape[patch.pitch_dim]
    assert nspan != npitch, "a wrong-length 1-D array must be distinguishable"

    if kind == "pitchwise":
        shape = [1, 1, 1]
        shape[patch.pitch_dim] = npitch
        value = np.full(shape, P_MEAN)
    elif kind == "full_face":
        value = np.full(patch.shape, P_MEAN)
    elif kind == "wrong_length":
        # 1-D is the spanwise spelling, so a length that is not the span count
        # is not a profile of anything.
        value = np.full((npitch,), P_MEAN)
    else:
        value = np.full(patch.shape + (3,), P_MEAN)

    with pytest.raises(ValueError, match="not a valid prescription"):
        patch.set_P(value)
...
def _backflow_patch(block, P=P_MEAN, sigma=1.0, backflow=True):
    ...
    patch = OutletPatch(i=-1, label="outlet_nrbc")
    block.patches.append(patch)
    patch.set_P(P)
    patch.sigma = sigma
    if backflow:
        _set_backflow(patch, *_snapshot())
    return patch
...
def test_set_backflow_Po_To_matches_the_state_it_names():
    """A stagnation state fills the same two rows as the enthalpy-entropy form.

    The conversion is the boundary condition's, so the two spellings have to
    land on the same target rather than merely near it.
    """
    _, patch = _attached()
    Po, To = 1.3e5, 340.0

    patch.set_backflow_Po_To(Po, To)
    got = _backflow_state(patch)[0:2]

    rhoo_nd, uo_nd = FLUID.set_P_T(Po / FLUID.P_ref, To / FLUID.T_ref)
    expect = (
        float(FLUID.get_h(rhoo_nd, uo_nd)),
        float(FLUID.get_s(rhoo_nd, uo_nd)),
    )
...
def test_set_backflow_Po_To_overrides_set_backflow_ho_s():
    """Two spellings of the same rows: the last call wins, and survives replay.

    Both are recorded for update_ref_scales to replay, so the order they are
    ...
    # Force the replay: a new fluid re-runs every recorded setter in order.
    block.set_fluid(FLUID.change_ref(rho_ref=0.9, V_ref=120.0))
    replayed = np.copy(_backflow_state(patch)[0:2])

    fluid = block.fluid
    rhoo_nd, uo_nd = fluid.set_P_T(1.3e5 / fluid.P_ref, 340.0 / fluid.T_ref)
    np.testing.assert_allclose(
        replayed,
        (float(fluid.get_h(rhoo_nd, uo_nd)), float(fluid.get_s(rhoo_nd, uo_nd))),
        rtol=1e-5,
    )
    assert not np.allclose(after_Po_To, replayed)


@pytest.mark.parametrize(
    "Po, To, match",
    [
        (np.nan, 340.0, "Po must be finite"),
        (1.3e5, np.inf, "To must be finite"),
        (-1.0, 340.0, "Po must be positive"),
        (1.3e5, 0.0, "To must be positive"),
    ],
)
def test_set_backflow_Po_To_rejects_invalid(Po, To, match):
    """A stagnation state has to be positive and finite to be converted."""
    _, patch = _attached()
    with pytest.raises(ValueError, match=match):
        patch.set_backflow_Po_To(Po, To)
...
def test_backflow_runs_through_the_solver_loop():
    ...
    inlet.set_Beta(float(state.Beta))
    outlet.set_P(P_MEAN)
    _set_backflow(outlet, *_snapshot())
...
#
# A RevolutionPatch method rather than an outlet one, but an attached outlet
# is the handiest carrier for it and this is where its tests have always
# lived.
# ---------------------------------------------------------------------------
...
@pytest.fixture
def smoothing_patch():
    """An attached outlet, used only as a carrier for the pitch smoother."""
    # An even pitch count, so test_smooth_pitch_121_kills_sawtooth has a
    # clean Nyquist mode to annihilate.
    _, patch = _attached(npitch=16)
    return patch


def test_smooth_pitch_121_preserves_constant(smoothing_patch):
    """A pitch-uniform field passes through unchanged."""
    field = np.ones(smoothing_patch.block_view.shape, dtype=np.float32) * 2.5
    out = smoothing_patch.smooth_pitch_121(field, alpha=1.0)
    np.testing.assert_allclose(out, field)


def test_smooth_pitch_121_kills_sawtooth(smoothing_patch):
    """alpha=1 annihilates the pitch Nyquist (sawtooth) mode."""
    shape = smoothing_patch.block_view.shape
    pd = smoothing_patch.pitch_dim
    npitch = shape[pd]
...
def test_smooth_pitch_121_alpha_blend(smoothing_patch):
    """alpha in (0, 1) blends raw field with the smoothed one."""
    rng = np.random.default_rng(0)
    field = rng.standard_normal(smoothing_patch.block_view.shape).astype(np.float32)
    alpha = np.float32(0.95)
    full = smoothing_patch.smooth_pitch_121(field, alpha=1.0)
    blended = smoothing_patch.smooth_pitch_121(field, alpha=alpha)
    np.testing.assert_allclose(blended, alpha * full + (1.0 - alpha) * field, rtol=1e-6)


def test_smooth_pitch_121_alpha_zero_is_noop(smoothing_patch):
    """alpha=0 leaves the field unchanged."""
    rng = np.random.default_rng(1)
    field = rng.standard_normal(smoothing_patch.block_view.shape).astype(np.float32)
    out = smoothing_patch.smooth_pitch_121(field, alpha=0.0)
    assert out is field
