...
def test_set_targets_rejects_bad_shape():
    """Values that are not a scalar or a spanwise profile are refused."""
    block = make_block()
    patch = InletPatch(i=0)
    block.patches.append(patch)
...
def test_set_targets_rejects_a_pitchwise_profile():
    """A pitchwise-varying prescription is refused rather than silently averaged."""
    block = make_block()
    patch = InletPatch(i=0)
    block.patches.append(patch)
    shape = [1, 1, 1]
    shape[patch.pitch_dim] = patch.shape[patch.pitch_dim]
    with pytest.raises(ValueError, match="not a valid prescription"):
        patch.set_ho_s(np.full(shape, 3.0e5), 10.0)


@pytest.mark.parametrize("span_dim", [1, 2])
def test_set_targets_accept_a_bare_1d_span_profile(span_dim):
    """A plain length-nspan array is a spanwise profile, whichever axis the span is on.

    Parametrised over the orientation because the 1-D form has to be laid onto
    the patch's span axis explicitly; left to numpy's trailing-axis alignment
    it would land on the pitch for one of the two.
    """
    block = make_block(span_dim=span_dim, nspan=6, npitch=13)
    patch = InletPatch(i=0)
    block.patches.append(patch)
    nspan = patch.shape[patch.span_dim]
    Alpha = np.linspace(-10.0, 10.0, nspan)

    patch.set_Alpha(Alpha)

    np.testing.assert_allclose(
        patch.tanAlpha.ravel(), np.tan(np.radians(Alpha)), rtol=1e-5
    )
    assert patch.tanAlpha.shape[patch.pitch_dim] == 1
